...
- **Ответы по полным материалам** — транскрипты видео + комменты внутри ноутбуков,
  а не lossy-выжимка.
- **0 токенов модели** — Claude в рантайме нет; сервер просто шеллит `nlm`.
  Ответы — verbatim, по одному на ноутбук (без LLM-склейки).

```bash
./web/deploy.sh          # build + run контейнер yt2nlm-web на host :8091
# → http://localhost:8091/
```
Контейнер (`python:slim` + `notebooklm-mcp-cli`, без браузера — `nlm` ходит по
HTTP через куки) монтирует профиль `nlm` (`~/.notebooklm-mcp-cli`) и `state/` (:ro),
стоит на `--restart unless-stopped`. Сервер — чистый stdlib (`web/server.py`),
зависимостей сверх `nlm` нет. Запросы сериализованы с паузой (анти rate-limit).
